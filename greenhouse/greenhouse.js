document.addEventListener('DOMContentLoaded', () => {
    ...
    function initGame() {
        ...
        if (currentConfig.timePaused) {
            ...
            gameLoopInterval = setInterval(gameLoop, gameConfig.realSecondsPerGameHour * 3000);
        }
    ...
    function handleAction(action) {
        ...
        const actionsConfig = {
            ...
            'fanOn': () => { setDeviceState('fan', true); sendSignal('phunsuong1'); },
            'fanOff': () => { setDeviceState('fan', false); sendSignal('phunsuong2'); },
...
async function saveScore(finalScore) {
    const gameName = 'Trồng Cà Chua'; // <-- BẠN CÓ THỂ ĐẶT TÊN BẤT KỲ CHO GAME NÀY
    ...
    try {
        ...
        if (result.status === 'success') {
            console.log('Lưu điểm thành công:', result.message);
        } else {
            console.error('Lỗi khi lưu điểm:', result.message);
        }
    } catch (error) {
        console.error('Không thể gửi điểm đến máy chủ:', error);
    }
}
document.getElementById('btn_back').addEventListener('click', () => {
  window.location.href = '../index.php'; // Hoặc trang menu chính của bạn
});